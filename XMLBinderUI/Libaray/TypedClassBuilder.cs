...
namespace XMLBinderUI.Libaray  
{  
   public class TypedClassBuilder  
    {  
       AssemblyName asemblyName;
       public TypedClassBuilder(string ClassName)  
       {  
       ...
       public static object createInstance(Type TYPED, object[] data){
           object instance = (Activator.CreateInstance(TYPED));

           int count = 0;
           foreach (PropertyInfo propertyInfo in instance.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
           {
               string propName = propertyInfo.Name;
               PropertyInfo T_instance = TYPED.GetProperty(propName);
                T_instance.SetValue(instance, data[count], null);
                count++;
           }
